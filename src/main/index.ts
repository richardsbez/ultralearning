...
import { readFileSync, writeFileSync, readdirSync, existsSync, mkdirSync, unlinkSync, writeFile as writeFileAsync } from 'fs'
...
ipcMain.handle('write-file', (_event, filePath: string, content: string) => {
  return new Promise<boolean>((resolve, reject) => {
    ...
    writeFileAsync(filePath, content, 'utf-8', (err) => {
      if (err) reject(err)
      else resolve(true)
    })
  })
...
ipcMain.handle('open-temp-file', async (_event, base64Data: string, fileName: string) => {
  const { tmpdir } = await import('os')
  const { join: pathJoin } = await import('path')
  const tmpPath = pathJoin(tmpdir(), fileName)
