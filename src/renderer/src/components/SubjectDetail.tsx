...
export function SubjectDetail({ data, onChange }: Props) {
  ...
  const dataR    = useRef(data);    dataR.current    = data
  const onChangeR = useRef(onChange); onChangeR.current = onChange

  const update = useCallback((partial: Partial<SubjectData>) =>
    onChangeR.current({ ...dataR.current, ...partial }), [])

  const updatePrinciple = useCallback((key: PrincipleKey, pd: SubjectData['principles'][typeof key]) => {
    onChangeR.current({ ...dataR.current, principles: { ...dataR.current.principles, [key]: pd } })
  }, [])

  const handleRoadmapChange = useCallback((roadmap: RoadmapNode[]) =>
    updatePrinciple('meta', { ...dataR.current.principles.meta, roadmap })
  , [])
