...
function getNodeH(n: RoadmapNode): number {
  if (n.nodeType === 'checklist') {
    if (n.collapsed) return 36
    return n.h ?? (NH_CL_HDR + (n.checkItems?.length ?? 0) * NH_CL_ITEM + NH_CL_ADD)
  }
  if (n.nodeType === 'text')  return n.h ?? 100
  if (n.nodeType === 'image') return n.h ?? 120
  if (n.nodeType === 'group') return n.h ?? 160
  return n.h ?? NH_DEFAULT
}
function isResizable(n: RoadmapNode) {
  return true // all node types are resizable
}
...
export function Roadmap({ nodes, onChange }: Props) {
  ...
  // Ref to the world+SVG containers for direct DOM pan (avoids React re-render on every pixel)
  const worldRef = useRef<HTMLDivElement>(null)
  const svgWorldRef = useRef<SVGGElement>(null)

  const applyTransformDOM = useCallback((p:{x:number;y:number}, z:number) => {
    const t = `translate(${p.x}px,${p.y}px) scale(${z})`
    if (worldRef.current)    worldRef.current.style.transform = t
    if (svgWorldRef.current) svgWorldRef.current.style.transform = t
  }, [])

  const [sel, setSel] = useState<Set<string>>(new Set())   // kept for copySelected/paste/deleteSelected logic
  const selR = useRef(sel); selR.current = sel

  // ── DOM-based selection — ZERO React re-renders on select/deselect ─────────
  const setSelDOM = useCallback((ids: Set<string>) => {
    selR.current.forEach(id => {
      document.querySelector(`[data-node="${id}"]`)?.classList.remove('rm-node-sel')
    })
    ids.forEach(id => {
      document.querySelector(`[data-node="${id}"]`)?.classList.add('rm-node-sel')
    })
    selR.current = ids
    // Do NOT call setSel — that would trigger React re-render
  }, [])

  const livePosR  = useRef<Record<string,{x:number;y:number}>>({})
  const edgeRafR  = useRef<number|null>(null)

  // Move dragged nodes via DOM, update edges via rAF — zero React re-renders
  const rerender = useCallback(()=>{
    Object.entries(livePosR.current).forEach(([id,{x,y}])=>{
      const el = document.querySelector<HTMLElement>(`[data-node="${id}"]`)
      if (el) { el.style.left = x+'px'; el.style.top = y+'px' }
    })
  },[])

  const updateEdgesDOM = useCallback(()=>{
    const live = livePosR.current
    const ns   = nodesR.current
    ns.forEach(child=>{
      if (!child.parentId) return
      const parent = ns.find(n=>n.id===child.parentId); if (!parent) return
      const pp = live[parent.id]??{x:parent.x,y:parent.y}
      const cp = live[child.id] ??{x:child.x, y:child.y}
      const ph=getNodeH(parent), ch2=getNodeH(child), pw=getNodeW(parent), cw=getNodeW(child)
      const {sp,tp,ss,ts}=getBestPorts(pp.x,pp.y,ph,cp.x,cp.y,ch2,pw,cw)
      const d=bezierD(sp,tp,ss,ts), pts=arrowPts(tp,ts)
      const midX=(sp.x+tp.x)/2, midY=(sp.y+tp.y)/2
      const g=svgWorldRef.current?.querySelector<SVGGElement>(`[data-edge="${child.id}"]`)
      if (!g) return
      g.querySelectorAll('path').forEach(p=>p.setAttribute('d',d))
      g.querySelector('polygon')?.setAttribute('points',pts)
      const c=g.querySelector('circle'); c?.setAttribute('cx',String(midX)); c?.setAttribute('cy',String(midY))
      const tx=g.querySelector('text'); tx?.setAttribute('x',String(midX)); tx?.setAttribute('y',String(midY+4))
    })
  },[])

  const startEdgeRaf = useCallback(()=>{
    const tick=()=>{ updateEdgesDOM(); if(nodeDragR.current) edgeRafR.current=requestAnimationFrame(tick) }
    edgeRafR.current = requestAnimationFrame(tick)
  },[updateEdgesDOM])

  const stopEdgeRaf = useCallback(()=>{
    if(edgeRafR.current){cancelAnimationFrame(edgeRafR.current);edgeRafR.current=null}
  },[])
  ...
  // ── DOM-based hover — zero React re-renders ──────────────────────────────────
  const hoveredR = useRef<string|null>(null)
  const setHoveredDOM = useCallback((id: string|null) => {
    if (hoveredR.current) {
      document.querySelector(`[data-node="${hoveredR.current}"]`)?.classList.remove('rm-node-hovered')
    }
    hoveredR.current = id
    if (id) {
      document.querySelector(`[data-node="${id}"]`)?.classList.add('rm-node-hovered')
    }
  }, [])
  const [linkMode,  setLinkMode]  = useState(false)
  const linkModeR = useRef(linkMode); linkModeR.current = linkMode
  const draftR      = useRef<LinkDraft|null>(null)
  const draftSvgRef = useRef<SVGGElement>(null)   // direct DOM update for link preview

  const rerenderDraft = useCallback(()=>{
    const d = draftR.current
    const g = draftSvgRef.current
    if (!g) return
    if (!d) { g.style.display='none'; return }
    const src = nodesR.current.find(n=>n.id===d.fromId)
    if (!src) { g.style.display='none'; return }
    const sp2 = livePosR.current[src.id]??{x:src.x,y:src.y}
    const ph2 = getNodeH(src), pw2 = getNodeW(src)
    const isValid = d.over&&d.over!==d.fromId&&!hasCycle(nodesR.current,d.fromId,d.over)
    const color = d.over?(isValid?'#60a5fa':'#f87171'):'#60a5fa'
    const {sp:sp3}=getBestPorts(sp2.x,sp2.y,ph2,d.mx-NW_DEFAULT/2,d.my-NH_DEFAULT/2,NH_DEFAULT,pw2,NW_DEFAULT)
    const preD=`M ${sp3.x} ${sp3.y} L ${d.mx} ${d.my}`
    const pts=`${d.mx},${d.my} ${d.mx-AL},${d.my-AH} ${d.mx-AL},${d.my+AH}`
    g.style.display=''
    const path=g.querySelector('path')
    const poly=g.querySelector('polygon')
    if(path){path.setAttribute('d',preD);path.setAttribute('stroke',color)}
    if(poly){poly.setAttribute('points',pts);poly.setAttribute('fill',color)}
  },[])

  const boxStartR = useRef<{sx:number;sy:number;_lastSet?:number}|null>(null)
  const [box,       setBox]     = useState<BoxSel|null>(null)
  const boxR = useRef(box); boxR.current = box
  ...
  const saveAndChangeR = useRef(saveAndChange); saveAndChangeR.current = saveAndChange

  const undo = useCallback(()=>{
    if (histIdxR.current<=0) return
    histIdxR.current--; onChange(histR.current[histIdxR.current])
    setSelDOM(new Set()); livePosR.current={}
  },[onChange])
  ...
  const applyZoom = useCallback((nz:number,px:number,py:number)=>{
    ...
    const np={x:px-(px-x)*(z1/z0), y:py-(py-y)*(z1/z0)}
    panR.current=np; zoomR.current=z1
    applyTransformDOM(np,z1)
    setPan(np); setZoom(z1)
  },[applyTransformDOM])
  ...
  const resetView = useCallback(()=>{const np={x:60,y:60};panR.current=np;zoomR.current=1;applyTransformDOM(np,1);setPan(np);setZoom(1)},[applyTransformDOM])
  ...
  const getPos = useCallback((n:RoadmapNode)=>livePosR.current[n.id]??{x:n.x,y:n.y},[])
  ...
  const deleteSelected = useCallback(()=>{
    ...
    saveAndChange(nodesR.current.filter(n=>!dead.has(n.id))); setSelDOM(new Set())
  },[saveAndChange])
  ...
  const duplicateSelected = useCallback(()=>{
    ...
    saveAndChange([...nodesR.current,...duped]); setSelDOM(new Set(duped.map(n=>n.id))); setCtxMenu(null)
  },[saveAndChange])
  ...
  const paste = useCallback(()=>{
    ...
    saveAndChange([...nodesR.current,...pasted]); setSelDOM(new Set(pasted.map(n=>n.id)))
  ...
  const openCtx = useCallback((nodeId:string,e:React.MouseEvent)=>{
    ...
    const items:CtxItem[]=[
      ...
      {label:`⧉  ${t.rmCtxDuplicate}`, action:()=>{setSelDOM(new Set([nodeId]));duplicateSelected()}},
      {label:`✂  ${t.rmCtxDisconnect}`, action:()=>{saveAndChange(nodesR.current.map(n=>n.id===nodeId?{...n,parentId:null}:n));setCtxMenu(null)}},
      {label:`×  ${t.rmCtxDelete}`, action:()=>{setSelDOM(new Set([nodeId]));setCtxMenu(null);setTimeout(deleteSelected,0)}, danger:true},
    ]
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      switch(true){
        case e.key==='Escape': setSelDOM(new Set());setLinkMode(false);draftR.current=null;rerenderDraft();setCtxMenu(null);if(editing)setEditing(null);break
        ...
        case meta&&e.key==='a'&&!inTextField: e.preventDefault();setSelDOM(new Set(nodesR.current.map(n=>n.id)));break
  ...
  // ── Global mouse ──────────────────────────────────────────────────────────────
  const resizeLiveR = useRef<{id:string;w:number;h:number}|null>(null)

  useEffect(()=>{
    const move=(e:MouseEvent)=>{
      if (panDragR.current){
        const {sx,sy,ox,oy}=panDragR.current
        const np={x:ox+e.clientX-sx, y:oy+e.clientY-sy}
        panR.current = np
        applyTransformDOM(np, zoomR.current)
      }
      if (nodeDragR.current&&!linkModeR.current){
        ...
        // Edge rAF started on mousedown; it reads livePosR every frame
      }
      if (resizeDragR.current){
        ...
        resizeLiveR.current={id,w:nw,h:nh}
        // Apply size directly to the DOM node (no React re-render during drag)
        const el=document.querySelector<HTMLElement>(`[data-node="${id}"]`)
      ...
      if (boxStartR.current){
        const r=canvasRef.current?.getBoundingClientRect()
        if(r) {
          const newBox={sx:boxStartR.current.sx,sy:boxStartR.current.sy,ex:e.clientX-r.left,ey:e.clientY-r.top}
          // Throttle box updates to every 2 frames
          if (!boxStartR.current._lastSet || e.timeStamp - boxStartR.current._lastSet > 32) {
            boxStartR.current._lastSet = e.timeStamp
            setBox(newBox)
          }
        }
      }
    ...
    const up=(e:MouseEvent)=>{
      if (panDragR.current){
        // Commit pan to React state once on mouseUp
        setPan({...panR.current})
        panDragR.current=null; return
      }
      if (resizeDragR.current){
        // Commit resize once on mouseUp
        if (resizeLiveR.current) {
          const {id,w,h}=resizeLiveR.current
          saveAndChangeR.current(nodesR.current.map(n=>n.id===id?{...n,w,h}:n))
          resizeLiveR.current=null
        }
        resizeDragR.current=null; return
      }
      if (nodeDragR.current){
        const pos={...livePosR.current}
        stopEdgeRaf()
        saveAndChangeR.current(nodesR.current.map(n=>pos[n.id]?{...n,...pos[n.id]}:n))
        livePosR.current={}; nodeDragR.current=null; return
      }
      if (draftR.current&&!draftR.current.over){draftR.current=null;rerenderDraft();return}
      if (boxStartR.current){
        const r=canvasRef.current?.getBoundingClientRect()
        const b=boxR.current
        if(r&&b){
          ...
          } else { if(!e.shiftKey) setSelDOM(new Set()) }
        }
  ...
  // Sort: groups first (behind), then others — memoized
    ...
    <div className="rm-wrapper">
      ...
      <div className="rm-toolbar">
        ...
        <label className="rm-btn rm-img-label" title={t.rmNewImage}>
          ...
          <input type="file" accept="image/*" style={{display:'none'}} onChange={async e=>{
            ...
            if (snapGridR.current){p.x=snapG(p.x);p.y=snapG(p.y)}
            const node:RoadmapNode={id:genId(),title:'imagem',status:'todo',notes:'',parentId:null,nodeType:'image',w:220,h:150,imageData:data,...p}
            saveAndChange([...nodesR.current,node]); setSelDOM(new Set([node.id]))
            e.target.value=''
          }}/>
        </label>
      ...
      <div
        ...
        onMouseDown={e=>{
          ...
          if (isBg){
            if (!e.shiftKey) setSelDOM(new Set())
            const r=canvasRef.current!.getBoundingClientRect()
      ...
      >
        ...
        <svg className="rm-svg" style={{position:'absolute',inset:0,width:'100%',height:'100%',overflow:'visible',pointerEvents:'none'}}>
          <g ref={svgWorldRef} style={{transform:wt,transformOrigin:'0 0'}}>
            {nodes.map(child=>{
              ...
              const midX=(sp.x+tp.x)/2, midY=(sp.y+tp.y)/2
              return (
                <g key={child.id} data-edge={child.id}>
            ...
            {/* permanent draft preview element — updated via DOM ref, no React re-render */}
            <g ref={draftSvgRef} style={{display:'none'}}>
              <path fill="none" strokeWidth={2} strokeDasharray="7 4" opacity={0.85}/>
        ...
        {/* HTML nodes */}
        <div ref={worldRef} className="rm-world" style={{transform:wt,transformOrigin:'0 0'}}>
          {sortedNodes.map(node=>{
            ...
            const borderColor = isSrc?'#60a5fa':isOver&&canConn?'#60a5fa':isOver&&!canConn?'#f87171':isGroup?(node.color??'#2a2a2a'):s.border
            const borderW2 = isSrc||isOver?'2px':'1.5px'
            ...
            return (
              <div
                ...
                style={{
                  ...
                  width:nodeW,
                  height: (isGroup||isText||isImage) ? nodeH
                        : isCL ? (node.collapsed ? 36 : (node.h ? nodeH : 'auto'))
                        : (node.h ? node.h : 'auto'),
                  minHeight: isCL ? (node.collapsed ? undefined : NH_CL_HDR+NH_CL_ADD) : (!isGroup&&!isText&&!isImage) ? NH_DEFAULT : undefined,
                  background:isGroup?'rgba(255,255,255,0.02)':isImage?'#111':s.bg,
                  border:`${borderW2} solid ${borderColor}`,
                  boxShadow:isSrc?`0 0 0 3px rgba(96,165,250,0.2)`:undefined,
                  cursor:linkMode?(draft?'crosshair':'grab'):isEdit?'text':'grab',
                  zIndex:isGroup?0:isSrc?20:1,
                ...
                onMouseDown={e=>{
                  ...
                  if (!selR.current.has(node.id)){
                    if (e.shiftKey) setSelDOM(new Set([...selR.current, node.id]))
                    else setSelDOM(new Set([node.id]))
                  }
                  const ids=selR.current.has(node.id)?[...selR.current]:[node.id]
                ...
                onMouseEnter={()=>{
                  setHoveredDOM(node.id)
                  if(draftR.current){draftR.current={...draftR.current,over:node.id};rerenderDraft()}
                }}
                onMouseLeave={()=>{
                  setHoveredDOM(null)
                  if(draftR.current){draftR.current={...draftR.current,over:null};rerenderDraft()}
                }}
              ...
              >
                ...
                {!isGroup&&!isText&&!isImage&&!isCL&&(
                  <>
                    <button className="rm-node-dot" style={{background:accent,flexShrink:0,marginLeft:node.color?10:undefined,alignSelf:'flex-start',marginTop:4}} title={s.label} onClick={e=>cycleStatus(node.id,e)} onMouseDown={e=>e.stopPropagation()}/>
                    {isEdit?(
                      <textarea ref={textareaR as React.RefObject<HTMLTextAreaElement>} className="rm-node-input rm-node-textarea" value={editVal} onChange={e=>setEditVal(e.target.value)} onBlur={commitEdit} onKeyDown={e=>{if(e.key==='Escape')setEditing(null)}} onClick={e=>e.stopPropagation()} onMouseDown={e=>e.stopPropagation()} placeholder="título do nó"/>
                    ):(
                    ...
                    <div className="rm-resize-handle" onMouseDown={e=>{e.stopPropagation();resizeDragR.current={id:node.id,sx:e.clientX,sy:e.clientY,ow:nodeW,oh:node.h??NH_DEFAULT}}}/>
                  </>
                )}
                ...
                {isCL&&(
                  <>
                    <div className="rm-cl-header" onMouseDown={e=>e.stopPropagation()}>
                      {/* Collapse toggle */}
                      <button
                        className="rm-cl-collapse"
                        onClick={e=>{e.stopPropagation();saveAndChange(nodesR.current.map(n=>n.id===node.id?{...n,collapsed:!n.collapsed}:n))}}
                        onMouseDown={e=>e.stopPropagation()}
                      ...
                      <span className="rm-cl-count" style={{color: (node.checkItems?.filter(ci=>ci.checked).length??0)===(node.checkItems?.length??0)&&(node.checkItems?.length??0)>0?'#a3e635':undefined}}>
                        {node.checkItems?.filter(ci=>ci.checked).length??0}/{node.checkItems?.length??0}
                      </span>
                    </div>
                    {!node.collapsed&&(
                      <>
                        ...
                        <div className="rm-cl-items" onMouseDown={e=>e.stopPropagation()}>
                          {(node.checkItems??[]).map(ci=>(
                            <div key={ci.id} className="rm-ci-row">
                              ...
                              <textarea
                                className="rm-ci-input rm-ci-textarea"
                                value={ci.text}
                                rows={1}
                                style={{textDecoration:ci.checked?'line-through':'none',color:ci.checked?'var(--muted)':'var(--text)'}}
                                onChange={e=>{
                                  const el=e.target; el.style.height='auto'; el.style.height=el.scrollHeight+'px'
                                  updateCheckItemText(node.id,ci.id,e.target.value.replace(/\n/g,''))
                                }}
                                onFocus={e=>{const el=e.target;el.style.height='auto';el.style.height=el.scrollHeight+'px'}}
                                onClick={e=>e.stopPropagation()}
                                onMouseDown={e=>e.stopPropagation()}
                                onKeyDown={e=>{
                                  if(e.key==='Enter'){e.preventDefault();addCheckItem(node.id)}
                                  if(e.key==='Backspace'&&ci.text===''){e.preventDefault();removeCheckItem(node.id,ci.id)}
                                }}
                              />
                        ...
                        <div className="rm-resize-handle" onMouseDown={e=>{e.stopPropagation();resizeDragR.current={id:node.id,sx:e.clientX,sy:e.clientY,ow:nodeW,oh:node.h??(NH_CL_HDR+(node.checkItems?.length??0)*NH_CL_ITEM+NH_CL_ADD)}}}/>
                      </>
                    )}
                    {!linkMode&&<button className="rm-node-add" title="Filho" onClick={e=>{e.stopPropagation();addNode(node.id)}} onMouseDown={e=>e.stopPropagation()}>+</button>}
                  </>
                ...
                {/* Delete button — always rendered, shown via CSS .rm-node-hovered */}
                {!isEdit&&!linkMode&&(
                  <button className="rm-node-del" onClick={e=>{e.stopPropagation();setSelDOM(new Set([node.id]));deleteSelected()}} onMouseDown={e=>e.stopPropagation()}>×</button>
                )}
                {/* Status badge — always rendered, shown via CSS .rm-node-hovered */}
                {!isGroup&&!isText&&!isImage&&!isCL&&(
                  <span className="rm-node-badge" style={{color:accent,borderColor:`${accent}40`}}>{s.label}</span>
                )}
                {linkMode&&<span className="rm-node-connect-hint rm-node-drag-hint">{t.rmDragHint}</span>}
