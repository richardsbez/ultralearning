...
export type ResourceType   = 'livro' | 'curso' | 'video' | 'artigo' | 'link' | 'pdf' | 'imagem' | 'audio' | 'outro'
export type ResourceStatus = 'na-fila' | 'em-uso' | 'concluido'

export interface ResourceAttachment {
  id: string
  name: string
  mimeType: string        // e.g. 'application/pdf', 'image/png', 'audio/mp3'
...
export interface Resource {
  ...
  fileData?: string          // legacy single file — kept for compatibility
  notes?: string
...
export interface RoadmapNode {
  ...
  collapsed?: boolean   // checklist/default: hide body, show only header
